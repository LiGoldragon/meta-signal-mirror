...
const MODULE_PATH: &[&str] = &["meta_signal_mirror", "lib"];
const SIGNAL_MODULE_PATH: &[&str] = &["signal_mirror", "lib"];
const IMPORTED_SIGNAL_TYPES: &[&str] = &["StoreName"];
const STANDARD_MODULE_PATH: &[&str] = &["signal_standard", "lib"];
const IMPORTED_STANDARD_TYPES: &[&str] = &["NetworkEndpoint", "SocketPath"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let ordinary_metadata = CargoEthosSourceMetadata::new("signal-mirror");
        ordinary_metadata.emit_dependency_rerun_instruction();
        let ordinary_source_path = ordinary_metadata
            .dependency_source_directory()
            .expect("signal-mirror publishes its Ethos source directory")
            .join("interface.ethos");
        let ordinary_source = fs::read_to_string(&ordinary_source_path)
            .expect("read the producer-owned ordinary Mirror Interface source");
        assert_eq!(
            ordinary_source,
            signal_mirror::MIRROR_INTERFACE_SOURCE,
            "Cargo metadata must resolve the exact source compiled by signal-mirror",
        );

        let standard_metadata = CargoEthosSourceMetadata::new("signal-standard");
        standard_metadata.emit_dependency_rerun_instruction();
        let standard_source_path = standard_metadata
            .dependency_source_directory()
            .expect("signal-standard publishes its Ethos source directory")
            .join("interface.ethos");
        let standard_source = fs::read_to_string(&standard_source_path)
            .expect("read the producer-owned shared vocabulary Interface source");
        assert_eq!(
            standard_source,
        ...
        let source = fs::read_to_string(&source_path).expect("read owner Mirror Interface source");
        ...
        .expect("assemble authority-approved owner Mirror Interface transaction");
        let rust = rust_logos();
        let type_paths = MirrorRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project owner Mirror Interface from the verified transaction")
            .write_or_check("META_SIGNAL_MIRROR_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in owner Mirror Interface source and Rust projection are fresh");
        CargoEthosSourceMetadata::new("meta-signal-mirror")
...
fn imported_signal_seats() -> Vec<&'static signal_mirror::bootstrap_manifest::DeclarationSeat> {
    let wanted = IMPORTED_SIGNAL_TYPES
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    let seats = signal_mirror::bootstrap_manifest::DECLARATION_SEATS
        .iter()
...
fn imported_standard_seats() -> Vec<&'static signal_standard::bootstrap_manifest::DeclarationSeat> {
    let wanted = IMPORTED_STANDARD_TYPES
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    ...
    assert_eq!(
        seats.len(),
        wanted.len(),
        "every shared vocabulary import is owned by signal-standard",
    );
    seats
}
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    let imported_signal = imported_signal_seats();
    let imported_standard = imported_standard_seats();
    let metadata = TextualMetadataSnapshot::new(
        specifications
            ...
            .chain(imported_signal.iter().map(|seat| {
                metadata_record(
                    SIGNAL_MODULE_PATH,
                    seat.spelling,
            ...
            .chain(imported_standard.iter().map(|seat| {
                metadata_record(
    ...
    let schemas = IdentitySchemaCatalog::new(
        specifications
            ...
            .chain(imported_standard.iter().map(|seat| {
                ...
                .expect("shared vocabulary types are admitted as imported nominal identities")
            }))
            .collect(),
    )
...
impl MirrorRustTypePaths {
    fn new() -> Self {
        ...
        for seat in imported_standard_seats() {
            let encoded = RustEncodedIdCodec::encode(&universal(seat.local));
            paths.insert(
                universal(seat.local),
                RustTypePath::try_new(vec![
